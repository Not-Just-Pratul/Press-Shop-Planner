...
export function PlannerControls({
  ...
  const handleAddConstraint = () => {
    setConstraints(prev => [
      ...prev,
      { id: crypto.randomUUID(), machineName: "", startTime: "", endTime: "" },
    ]);
  };
  ...
  const handleConstraintChange = (
    id: string,
    field: 'machineName' | 'startTime' | 'endTime',
    value: string,
  ) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const processedBreakTime: BreakTimeMinutes = {
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="space-y-2">
          <h3 className="font-semibold text-lg">
            {isAdjustingPlan ? 'Re-Run Planner' : 'Run Planner'}
          </h3>
        ...
        {isAdjustingPlan ? (
          <AdjustingPlanControls
            shiftStartTime={shiftStartTime}
            replanTime={replanTime}
            onReplanTimeChange={setReplanTime}
          />
        ) : (
          <NewPlanControls
            startTime={startTime}
            ...
            availableMachinesForConstraints={availableMachinesForConstraints}
        ...
        <div className="flex flex-wrap gap-4 items-center">
          <Button
            type="submit"
            disabled={isGeneratingPlan || (!isAdjustingPlan && totalShiftDuration <= 0)}
            size="lg"
          >
...
function AdjustingPlanControls({
  shiftStartTime,
  replanTime,
  onReplanTimeChange,
}: {
  shiftStartTime: string;
  replanTime: string;
  onReplanTimeChange: (value: string) => void;
}) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <div className="space-y-2">
        ...
        <Input
          ...
          type="time"
          value={replanTime}
          onChange={(e) => onReplanTimeChange(e.target.value)}
          required
        />
        <p className="text-xs text-muted-foreground">
          Enter the time to start the new plan from.
        </p>
      </div>
    </div>
  );
...
function NewPlanControls({
  startTime,
  endTime,
  breakTime,
  totalShiftDuration,
  constraints,
  availableMachinesForConstraints,
  onStartTimeChange,
  onEndTimeChange,
  onBreakTimeChange,
  onAddConstraint,
  ...
  availableMachinesForConstraints: Machine[];
  onStartTimeChange: (value: string) => void;
  onEndTimeChange: (value: string) => void;
  onBreakTimeChange: (updater: (prev: BreakTime) => BreakTime) => void;
  onAddConstraint: () => void;
  onRemoveConstraint: (id: string) => void;
  onConstraintChange: (id: string, field: 'machineName' | 'startTime' | 'endTime', value: string) => void;
}) {
  return (
    <div className="space-y-4">
      ...
      <p className="text-sm text-muted-foreground">
        ...
        <span className="font-semibold">
          {totalShiftDuration > 0 ? `${totalShiftDuration} minutes` : 'Invalid time range'}
        </span>
      </p>
        ...
        <CardContent className="flex flex-col sm:flex-row items-center gap-4">
          <div className="space-y-2 w-full sm:flex-1">
            ...
            <Input
              ...
              onChange={(e) => onBreakTimeChange(bt => ({ ...bt, start: e.target.value }))}
            />
          </div>
          <div className="space-y-2 w-full sm:flex-1">
            ...
            <Input
              ...
              type="time"
              value={breakTime.end}
              onChange={(e) => onBreakTimeChange(bt => ({ ...bt, end: e.target.value }))}
            />
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent className="space-y-4">
            ...
            <div key={constraint.id} className="grid grid-cols-1 sm:grid-cols-[1fr_auto_1fr_auto] gap-2 items-end">
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value) => onConstraintChange(constraint.id, 'machineName', value)}
                >
                  <SelectTrigger>
                    <SelectValue placeholder="Select machine" />
                  </SelectTrigger>
                  <SelectContent>
                    {constraint.machineName && (
                      <SelectItem value={constraint.machineName}>{constraint.machineName}</SelectItem>
                    )}
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => onConstraintChange(constraint.id, 'startTime', e.target.value)}
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => onConstraintChange(constraint.id, 'endTime', e.target.value)}
              ...
              <AlertDialog>
                ...
                <AlertDialogContent>
                  ...
                  <AlertDialogFooter>
                    <AlertDialogCancel>Cancel</AlertDialogCancel>
                    <AlertDialogAction onClick={() => onRemoveConstraint(constraint.id)}>
                      Remove
                    </AlertDialogAction>
                  </AlertDialogFooter>
                </AlertDialogContent>
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              ...
              <AlertDialogFooter>
                <AlertDialogCancel>Cancel</AlertDialogCancel>
                <AlertDialogAction onClick={onAddConstraint}>Add</AlertDialogAction>
              </AlertDialogFooter>
            </AlertDialogContent>
          </AlertDialog>
        </CardContent>
